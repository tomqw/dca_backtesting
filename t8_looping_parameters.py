...
from functions.bot_config import getMax, getSingleConfig
...
while config.deviation_to_open_safety_order <= 3:
    while config.safety_order_volume_scale <= 2:
        config.max_safety_order_price_deviation, config.max_amount_for_bot_usage = (
            getMax(config)
        )
        # print (config.to_json())
        ...
        print(
            ...
            config.max_amount_for_bot_usage,
            result["profit"],
            round(config.safety_order_volume_scale, 2),
            config.safety_order_step_scale,
            config.max_safety_order_price_deviation,
            round(config.deviation_to_open_safety_order, 2),
        )
